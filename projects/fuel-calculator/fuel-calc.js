...
function renderCars() {
  ...
  grid.innerHTML = cars
    .map((c) => {
      const iconHtml = c.icon.startsWith('http://') || c.icon.startsWith('https://')
...
function calculate() {
  ...
  document.getElementById('resultBody').innerHTML = `
    <div class="result-summary">
      <div class="summary-row">${destSummary}</div>
      <div class="summary-row">${selectedCar.name} (${selectedCar.eff} L/100km)</div>
      <div class="summary-row">$${fuelPrice.toFixed(2)}/L · ${passengers} ${passengers === 1 ? 'passenger' : 'passengers'}</div>
    </div>
